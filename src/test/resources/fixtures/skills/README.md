...
```bash
./gradlew test --no-daemon --console=plain --tests dev.dong4j.idea.skill.inspector.rules.SkillFixturesRuleRunnerTest
```

测试会执行以下流程:

1. 遍历 `src/test/resources/fixtures/skills/*/SKILL.md`
2. 读取 `expected-rule-ids.properties` 中每个 fixture 对应的预期 ruleId
3. 使用真实 Markdown/YAML PSI 构建 `SkillFile`
4. 注入 fixture 目录路径给 Reference / Resource 规则
5. 运行 `RuleRunner`
6. 断言实际触发的 ruleId 与预期完全一致

新增 fixture 时需要同时更新 `expected-rule-ids.properties`:

```properties
my-case=rule.id.one,rule.id.two
clean-case=
```

空值表示该 fixture 预期不触发任何问题。
...
| `invalid-yaml/`           | `frontmatter.invalid-yaml`                                | YAML frontmatter 故意缺少闭合分隔符；命中后 RuleRunner 停止后续规则，避免正文派生噪音                              |
