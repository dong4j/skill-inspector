...
- **编辑器浮动按钮 PoC**（`floating/SkillFloatingToolbarProvider`）：通过 IntelliJ 官方 `editorFloatingToolbarProvider`
  扩展点把 `SkillInspectorAction` 以悬浮按钮形态暴露在 SKILL.md 编辑器右上角，鼠标悬停淡入，与 IDE AI Assistant 风格一致；
  仅当文件名为 `SKILL.md` 时显示，与右键菜单共用同一份 Action 配置。
