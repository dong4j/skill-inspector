...
- **编辑器右下角浮动按钮**（`floating/SkillBottomFloatingButton` + `SkillBottomFloatingInstaller`）：
  自定义圆角矩形 FAB（icon + 文本 + 毛玻璃背景），通过 `AppLifecycleListener` 启动 +
  `EditorFactoryListener` 逐编辑器挂载到 `JLayeredPane#PALETTE_LAYER`，编辑器释放时自动清理。
  点击触发 `ValidateCurrentSkillFileAction` 仅校验当前文件，用 `SimpleDataContext`
  显式注入按钮关联的 editor/file/project，规避按钮挂 RootPane 时 DataContext 拿到 focus 编辑器
  文件的陷阱。**PoC 阶段曾对比过官方 `editorFloatingToolbarProvider`（右上角）方案**，
  验证后选定本路径，官方 Provider 路径已移除。
