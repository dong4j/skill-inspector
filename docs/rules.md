...
规则按确定性和风险分为四层，对应 `rules/` 包下四个实现类：
...
4. **Security Rules** (`SecurityRules`)：安全规则，发现高风险内容（疑似密钥、危险命令、过宽 `allowed-tools`、敏感路径、prompt injection）。
...
后两条目前没有对应规则实现，作为 `resource.unused-reference` / `script.missing-usage` 的待办依据。
