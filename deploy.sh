...
#
# 设计参考: skillsjars-helper/deploy.sh (同作者的姐妹插件部署脚本).
...
REMOTE_HOST="${SKILL_INSPECTOR_DEPLOY_HOST:-aliyun}"

# 远程站点 root 目录, 必须与 landing/nginx.conf 中 `root` 一致.
REMOTE_ROOT_DIR="${SKILL_INSPECTOR_DEPLOY_ROOT:-/var/www/skill-inspector}"

# 远程 nginx conf.d 目录 (一般不用改).
REMOTE_NGINX_CONF_DIR="${SKILL_INSPECTOR_DEPLOY_NGINX_DIR:-/etc/nginx/conf.d}"
...
PLUGIN_MARKETPLACE_ID="${SKILL_INSPECTOR_MARKETPLACE_ID:-31938}"
...
环境变量覆盖 (临时改一次部署目标特别有用, 不用改脚本):
  SKILL_INSPECTOR_DEPLOY_HOST       覆盖 REMOTE_HOST    (默认 aliyun)
  SKILL_INSPECTOR_DEPLOY_ROOT       覆盖 REMOTE_ROOT_DIR (默认 /var/www/skill-inspector)
  SKILL_INSPECTOR_DEPLOY_NGINX_DIR  覆盖远程 nginx conf.d 目录 (默认 /etc/nginx/conf.d)
  SKILL_INSPECTOR_DEPLOY_DATA_DIR   覆盖独立下载站目录 (默认空 = 跳过)
  SKILL_INSPECTOR_DEPLOY_SITE_URL   覆盖收尾输出的访问地址
  SKILL_INSPECTOR_MARKETPLACE_ID    Marketplace plugin ID (上架后填入)

示例:
  SKILL_INSPECTOR_DEPLOY_HOST=staging ./deploy.sh -d   # 临时部署到 staging 主机
...
echo "================================"
echo "✓ 部署流程结束"
$do_publish      && echo "  - Marketplace 默认通道: $MARKETPLACE_URL"
$do_publish_beta && echo "  - Marketplace beta 通道: $MARKETPLACE_URL (审核后可见)"
